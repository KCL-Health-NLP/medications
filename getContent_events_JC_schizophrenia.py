...
def produceDocuments(df):
    import os
    import shutil
    outputDir=NLP_dev+"annotation_projects_setup/output_example_schizophrenia/extraction_events"
 
    i = 0 #num_patients per batch
    j = 1 #num_batches (change to 6 if re-running for more batches)
    k = 1 #num_doc
    prev_brcid = "first"
    
    df=df.sort_values(by=['BrcId','ViewDate'])
    
    for t in df[['BrcId', 'CN_Doc_ID', 'Comments','ViewDate']].itertuples():
...
sql = ("SELECT distinct top 10000 NEWID() as UUID, a.BrcId, a.CN_Doc_ID, a.Comments,a.ViewDate, b.Primary_Diag "+
"from sqlcris.dbo.Event a "+
"INNER JOIN SQLCRIS.dbo.Diagnosis b "+
"ON a.BrcId = b.BrcId "+
"WHERE b.Primary_Diag LIKE '%F20%' AND a.Comments <> ''"+
"ORDER BY NEWID()" )
...
df['attach_tokens_num'] = df['Comments'].apply(get_tokens_num)

df.to_pickle("./events_top_1000_schizophrenia.pkl")


#1) read first referrals pickle with docs, length, average line length, num_symptoms   
pickle_in = open("events_top_1000_schizophrenia.pkl","rb")
...
print("Events with at least 5 tokens:", len(df_red))
df_red = df_all[df_all['attach_tokens_num']>=500]
print("Events with at least 500 tokens:", len(df_red))
...
df_len = (df_red.groupby('BrcId')['Comments'].apply(lambda x: np.mean(x.str.len())).reset_index(name='mean_len_text'))
print(df_len)
