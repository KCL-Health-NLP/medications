...
def batch_process_directory(pin, full_key=True):
    ...
    d_list = [os.path.join(pin, os.path.join(d, 'adjudication')) for d in os.listdir(pin) if 'config' not in d and 'annotationadmin' not in d and '.' not in d]
...
annotator = 'Chloe'
dir_1 = "S:/AchlysShared/BRC_CRIS/Natalia Viani/annotation_prescription/"+annotator+"/Depression/Events_done/adjudication/"
...
if(write_file):
    ...
    text_file = open("./prescriptions/depression/adjudication/output_"+annotator+"_batch1to5_new_adj_events"+".txt", "w")
...
for batch in batches:
    if(os.path.isdir(dir_1+'batch_'+batch)):
        ...
        for path, annotationList in annotations.items():
           ...
           #docPath = path.split('/')[8]
            docPath = ''
            m = re.search('batch.*xml', path)
            docPath = m.group(0)
            batch = docPath.split('\\')[0]
            pat = docPath.split('\\')[1]
            docName = docPath.split('\\')[3].split('.')[0]
            if(write_file):
                for key, value in annotationList.items():
                    ...
                    adjudication_files.append(pat+"\\"+docName)
        
        adjudication_files = list(dict.fromkeys(adjudication_files))
        missing = [s for s in all_files if s not in adjudication_files]
        print(batch, missing)
